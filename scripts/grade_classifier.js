...
class GradeClassifier {
  ...
  // Подсчет очков для каждого уровня
  scoreGrade(text, salary, experience) {
    ...
    // Проверка заголовка (наибольший вес)
    ...
    if (salary && salary.max) {
      for (const [grade, patterns] of Object.entries(this.patterns)) {
        if (patterns.salary_max) {
          if (salary.max <= patterns.salary_max) {
            scores[grade] += 4;
          }
        }
        if (patterns.salary_min && salary.max >= patterns.salary_min) {
          scores[grade] += 2;
        }
      }
    }
    ...
    if (experience) {
      for (const [grade, patterns] of Object.entries(this.patterns)) {
        if (patterns.experience_max && experience.max <= patterns.experience_max) {
          scores[grade] += 4;
        }
        if (patterns.experience_min && experience.min >= patterns.experience_min) {
          scores[grade] += 3;
        }
      }
    }
